...
function toggleDarkMode() {
  ...
  try { localStorage.setItem('lx_dark', state.darkMode ? '1' : '0'); } catch(e) {}
}

function initDarkMode() {
  let saved = '0';
  try { saved = localStorage.getItem('lx_dark') || '0'; } catch(e) {}
  if (saved === '1') {
    state.darkMode = true;
...
function setView(type) {
  ...
  document.getElementById('productGrid').classList.toggle('list-view', type === 'list');
...
function checkout() {
  ...
  const sum = state.cart.reduce((s, c) => s + c.price * c.qty, 0);
  showToast(`<i class="fa fa-credit-card"></i> ${formatPrice(sum)} 결제 페이지로 이동합니다`, 'success');
  logSecurityEvent('CHECKOUT_ATTEMPT', { total: sum, items: state.cart.length, timestamp: new Date().toISOString() });
  toggleCart();
}
...
function toggleWish(e, id) {
  ...
  if (idx >= 0) {
    ...
    showToast('<i class="fas fa-heart" style="color:#d32f2f"></i> 찜 목록에 추가되었습니다', 'success');
  }
